...
**监听器(**ApplicationListener) 对应于观察者模式中的**观察者**。监听器监听特定事件,并
...
那到底是推还是拉呢，从 Nacos 客户端**通过 Listener** 来接收最新数据进行分析
...
看名字就知道，这种负载均衡策略就是**随机选择一个服务实例**，看源码我们知道，在RandomRule的无参构造方法中初始化了一个Random对象，
...
RoundRobinRule这种负载均衡策略叫做线性**轮询负载均衡策略**。这个类的choose(ILoadBalancer lb, Object key)函数整体逻辑是这样的：开启
...
看名字就知道这种负载均衡策略带有**重试**功能。首先RetryRule中又定义了一个subRule，它的实现类是RoundRobinRule，然后在RetryRule的
...
WeightedResponseTimeRule中会根据每一个实例的运行情况来给计算出该实例的一个**权重**，然后在挑选实例的时候则根据权重进行挑选，这样能

够实现更优的实例调用。WeightedResponseTimeRule中有一个名叫DynamicServerWeightTask的定时任务，默认情况下每隔30秒会计算一次
...
choose方法，所以它的选择策略**和\****RoundRobinRule的选择策略一致**，不赘述。

**BestAvailableRule**

BestAvailableRule继承自ClientConfigEnabledRoundRobinRule，它在ClientConfigEnabledRoundRobinRule的基础上主要增加了根据

loadBalancerStats中保存的服务实例的状态信息来**过滤掉失效的服务实例的功能，然后顺便找出并发请求最小的服务实例来使用。**然而
...
put到:Map:BeanName,Bean实例,即一级缓存
...
![img](spring.assets/20230724024159.pngorigin_url=file%3A%2F%2FD%3A%2Finterview%2Fimg%2F202201041510573.jpg%3FlastModify%3D1710063657&pos_id=LfvEdoSO)
...
![img](spring.assets/20230724024159.pngorigin_url=file%3A%2F%2FD%3A%2Finterview%2Fimg%2F202202152023778.jpg%3FlastModify%3D1710063657&pos_id=2fJ4nh9X)
...
![img](spring.assets/20230724024159.pngorigin_url=file%3A%2F%2FD%3A%2Finterview%2Fimg%2F202202161506209.jpg%3FlastModify%3D1710063657&pos_id=hUcZUTtH)
...
![img](spring.assets/20230724024159.pngorigin_url=file%3A%2F%2FD%3A%2Finterview%2Fimg%2F202202211054208.jpg%3FlastModify%3D1710063657&pos_id=J9vLzjtd)
...
开始的时候注册EventListenerMethodProcessor处理器

调用smartSingleton.afterSingletonsInstantiated

循环所有实现了注解的Bean,然后创建事件监听器,并注册事件到Context中
...
![img](spring.assets/96b8de5d82923aacdfae70a0589ec9b4.png)

![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)编辑



![img](spring.assets/4a5b4816e189af90561e113268d5fcf5.png)
